...
class RoverMapping:
    ...
    def ar_cb(self, markers):
	    ...
	    self.H = matrix([[0, -1, 0],[0,0,1],[-1,0,0]]) # dimensionally this works out 
	    self.H2 = matrix([[0, 1, 0],[1,0,0],[0,0,1]]) # dimensionally this works out 
	
	    X = vstack([x,y,euler[2]])
          #  X=dot(dot([x,y,z],H),H2)
	    m_pose = PointStamped()
            m_pose.header = m.header
            m_pose.point = m.pose.pose.position
            m_pose = self.listener.transformPoint("/%s/ground"%self.name,m_pose)

	   # m_pose = self.listener.transformPoint("/odom",m_pose)
            Z = vstack([m_pose.point.x,m_pose.point.y])
	   # Z = dot(dot([m_pose.point.x,m_pose.point.y,m_pose.point.z],H),H2)

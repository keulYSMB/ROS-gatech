#!/usr/bin/env python
import roslib
roslib.load_manifest('floor_mapping')
...
class FloorMapper:
    def __init__(self):
        ...
        self.target_frame = rospy.get_param("~target_frame","/follower")
...
#	print str(proba.data)
        I = self.br.imgmsg_to_cv(proba,"mono8")
        self.proba = cv.CloneMat(I)
#	rospy.loginfo("showing proba image")
	cv.ShowImage("proba received",self.br.imgmsg_to_cv(proba,"mono8"))
#	rospy.loginfo("image shown")
...
#            print "header : " + str(proba.header)
	    self.listener.waitForTransform(proba.header.frame_id,self.target_frame,proba.header.stamp,rospy.Duration(1.0))
#            self.listener.waitForTransform(proba.header.frame_id,'/world',proba.header.stamp,rospy.Duration(1.0))
	    trans = numpy.mat(self.listener.asMatrix(self.target_frame,proba.header))
            ...
            msg = self.br.cv_to_imgmsg(self.floor_map,"mono8")
